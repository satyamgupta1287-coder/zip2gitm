...
import { Upload, FileArchive, FolderTree, Code2, CheckCircle2, AlertCircle, RefreshCw, Plus, Trash2, FileText, X } from 'lucide-react';
...
export const WorkspaceManager: React.FC<WorkspaceManagerProps> = ({
  ...
  const handleSingleFileUpload = async (file: File) => {
    ...
    const isBinary = file.type ? !file.type.startsWith('text/') && !file.type.includes('json') : false; // simple check
      ...
      {zipAnalysis && !isCreatingFile && (
        <div className="space-y-4">
             ...
             <div className="flex items-center space-x-3">
               ...
               <div>
                 ...
                 <p className="text-xs text-slate-400 mt-0.5">
                   Staged <strong className="text-slate-200">{zipAnalysis.totalFiles} files</strong>
                 </p>
               </div>
             </div>
             <div className="flex space-x-2">
          ...
          <div className="max-h-60 overflow-y-auto bg-slate-950 border border-slate-800 rounded-xl p-2 space-y-1">
             ...
             {zipAnalysis.files.length === 0 && (
               <div className="p-4 text-center text-xs text-slate-500">No files staged.</div>
             )}
          </div>
        </div>
      )}
